...
async def record_usage(
    user_id: str, model_name: str, tokens_in: int, tokens_out: int
) -> None:
    try:
        supabase = SupabaseClient().db

        model_res = await asyncio.to_thread(
            lambda: supabase.table("models").select("*").eq("model_name", model_name).execute()
        )
        ...
        if not model_res.data:
            ...
            if not insert_res.data:
                logger.error(
                    f"Failed to dynamically create model entry for: {model_name}"
                )
                return
        ...
        await asyncio.to_thread(
            lambda: supabase.table("usage_tracking").insert(
                {
                    "user_id": user_id,
                    "model_id": model_id,
                    "request_count": 1,
