"""
Input sanitization with proper normalization and security validation.
No revelar patrones internos al usuario.
"""
...
def sanitize_input(text: str) -> str:
    ...
    if len(normalized) > MAX_INPUT_LENGTH:
        raise PolicyViolationException(
            f"Entrada excede límite permitido"
        )
...
def _truncate_history(history: list, max_messages: int = MAX_HISTORY_MESSAGES) -> list:
    ...
    while len(filtered) > max_messages:
        if len(filtered) > 2 and filtered[0].get("role") == "user" and len(filtered) > 2:
            filtered.pop(0)
...
def validate_history_length(history: list) -> bool:
    ...
    if len(history) > MAX_HISTORY_MESSAGES:
        raise PolicyViolationException(
            f"Historial excede límite permitido"
        )
    ...
    if total_chars > MAX_CONTEXT_CHARS:
        raise PolicyViolationException(
            f"Contexto excede límite permitido"
        )
    return True