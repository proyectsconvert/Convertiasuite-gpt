"""
Risk scoring engine para evaluación semántica de inputs.
Reemplaza blacklist-based detection con scoring probabilístico.
"""
...
class RiskScorer:
    ...
    INJECTION_INDICATORS = [
        ...
        (r"(?i)(what|which)\s*(are|is)\s*(your\s*)?(system\s*)?(instructions|config)", 0.5),
    ...
    JAILBREAK_INDICATORS = [
        ...
        (r"(?i)you\s*are\s*(now\s*)?an?\s*(AI|assistant)\s*without\s*restrictions", 0.6),
        (r"(?i)do\s*anything\s*now", 0.7),
    ]

    def __init__(self):
        self._injection_patterns = [(re.compile(p, re.I), s) for p, s in self.INJECTION_INDICATORS]
        self._exfil_patterns = [(re.compile(p, re.I), s) for p, s in self.DATA_EXFILTRATION_INDICATORS]
        self._jailbreak_patterns = [(re.compile(p, re.I), s) for p, s in self.JAILBREAK_INDICATORS]
    ...
    def score(self, text: str) -> RiskScore:
        ...
        injection_score = self._calculate_signal_score(text_lower, self._injection_patterns, factors)
        exfil_score = self._calculate_signal_score(text_lower, self._exfil_patterns, factors)
        jailbreak_score = self._calculate_signal_score(text_lower, self._jailbreak_patterns, factors)
