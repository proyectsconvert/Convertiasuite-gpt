...
class CompositeMemoryRepository(IMemoryRepository):
    ...
    async def save_messages(self, session_id: str, messages: list) -> None:
        await self._cache.save_messages(session_id, messages)
        await self._db.save_messages(session_id, messages)
    ...
    async def update_session(self, session_id: str, title: str) -> None:
        await self._cache.update_session(session_id, title)
        await self._db.update_session(session_id, title)

    async def get_context_window(self, session_id: str, window_size: int) -> list:
        cached = await self._cache.get_context_window(session_id, window_size)
        if cached:
            return cached
        return await self._db.get_context_window(session_id, window_size)