...
import { 
  ...
  Globe, BarChart3, LogOut, PanelLeftClose, PanelLeft
} from 'lucide-react';
import { useAppStore, type AppView } from '@/store/appStore';
import { Button } from '@/components/ui/button';

const mainNav: { icon: typeof MessageSquare; label: string; view: AppView }[] = [
  { icon: MessageSquare, label: 'Chat IA', view: 'chat' },
  { icon: FileText, label: 'Documentos', view: 'documents' },
  { icon: Globe, label: 'Web Builder', view: 'webbuilder' },
  { icon: Presentation, label: 'Presentaciones', view: 'presentations' },
  { icon: Search, label: 'Búsqueda', view: 'search' },
];
...
export default function AppSidebar() {
  const { view, setView, sidebarOpen, toggleSidebar, chats, currentChatId, setCurrentChatId } = useAppStore();
      ...
      <div className="w-14 bg-sidebar border-r border-sidebar-border flex flex-col items-center py-4 gap-2">
        ...
        {mainNav.map((n) => (
          <button key={n.view} onClick={() => setView(n.view)}
            className={`p-2 rounded-lg transition-colors ${view === n.view ? 'bg-sidebar-accent text-sidebar-primary' : 'text-muted-foreground hover:bg-sidebar-accent'}`}>
            <n.icon className="w-4 h-4" />
          </button>
        ))}
        <div className="flex-1" />
        <button onClick={() => setView('settings')} className="p-2 rounded-lg text-muted-foreground hover:bg-sidebar-accent transition-colors">
  ...
  return (
    <div className="w-72 bg-sidebar border-r border-sidebar-border flex flex-col h-screen">
      ...
      <div className="px-3 mb-3">
        <button onClick={() => useAppStore.getState().setCommandOpen(true)}
      ...
      <div className="px-3 mb-3 space-y-0.5">
        {mainNav.map((n) => (
          <button key={n.view} onClick={() => setView(n.view)}
            className={`w-full flex items-center gap-2.5 px-3 h-9 rounded-lg text-sm transition-colors ${view === n.view ? 'bg-sidebar-accent text-sidebar-primary font-medium' : 'text-muted-foreground hover:bg-sidebar-accent hover:text-foreground'}`}>
            <n.icon className="w-4 h-4" />
            {n.label}
          </button>
        ))}
      ...
      {favorites.length > 0 && (
        <div className="px-3 mb-2">
          <button onClick={() => setFavoritesOpen(!favoritesOpen)} className="w-full flex items-center gap-2 px-1 py-1 text-xs font-semibold text-muted-foreground uppercase tracking-wider hover:text-foreground transition-colors">
            ...
            <div className="mt-1 space-y-0.5">
              {favorites.map((c) => (
                <button key={c.id} onClick={() => { setCurrentChatId(c.id); setView('chat'); }}
                  className={`w-full text-left px-3 py-1.5 rounded-lg text-sm truncate transition-colors ${currentChatId === c.id && view === 'chat' ? 'bg-sidebar-accent text-foreground' : 'text-muted-foreground hover:bg-sidebar-accent'}`}>
                  {c.title}
                </button>
              ))}
            </div>
      ...
      <div className="flex-1 overflow-y-auto px-3">
        <button onClick={() => setHistoryOpen(!historyOpen)} className="w-full flex items-center gap-2 px-1 py-1 text-xs font-semibold text-muted-foreground uppercase tracking-wider hover:text-foreground transition-colors mb-1">
        ...
        {historyOpen && grouped.map((g) => (
          <div key={g.label} className="mb-3">
            ...
            <div className="space-y-0.5">
              {g.items.map((c) => (
                <button key={c.id} onClick={() => { setCurrentChatId(c.id); setView('chat'); }}
                  className={`group w-full text-left px-3 py-1.5 rounded-lg text-sm truncate transition-colors flex items-center ${currentChatId === c.id && view === 'chat' ? 'bg-sidebar-accent text-foreground' : 'text-muted-foreground hover:bg-sidebar-accent'}`}>
      ...
      <div className="p-3 border-t border-sidebar-border">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="text-sm font-medium text-foreground truncate">Carlos Méndez</div>
            <div className="text-xs text-muted-foreground truncate">Pro Plan</div>
          </div>
          <button onClick={() => setView('settings')} className="p-1.5 rounded-lg hover:bg-sidebar-accent transition-colors">
