...
import {
  ...
  ChevronRight, Check, CreditCard, LogOut
} from 'lucide-react';
        ...
        <div className="space-y-0.5 flex-1">
          {sections.map((s) => (
            <button key={s.id} onClick={() => setActiveSection(s.id)}
              className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm transition-colors ${activeSection === s.id ? 'bg-secondary text-foreground font-medium' : 'text-muted-foreground hover:bg-secondary/50'}`}>
              <s.icon className="w-4 h-4" /> {s.label}
            </button>
          ))}
        </div>
        <button onClick={() => setView('landing')} className="flex items-center gap-2 px-3 py-2 rounded-lg text-sm text-destructive hover:bg-destructive/10 transition-colors">
            ...
            <div className="space-y-6">
              ...
              <div className="surface-card p-6 space-y-4">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <div className="text-lg font-semibold text-foreground">Carlos Méndez</div>
                    <div className="text-sm text-muted-foreground">carlos@empresa.com</div>
                  </div>
              ...
              <div className="surface-card p-6 space-y-6">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex bg-secondary rounded-lg p-0.5">
                    <button onClick={() => { if (darkMode) toggleDarkMode(); }}
                      className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-medium transition-colors ${!darkMode ? 'bg-background text-foreground shadow-sm' : 'text-muted-foreground'}`}>
                ...
                <div className="flex items-center justify-between mb-6">
                  <div>
                    <div className="flex items-center gap-2">
                      <span className="text-lg font-bold text-foreground">Plan Pro</span>
                    ...
                    <div key={item.label}>
                      <div className="flex items-center justify-between text-sm mb-1.5">
                        ...
                        <span className="text-muted-foreground">
                          {item.used}{item.unit ? item.unit : ''} / {item.total === 'Ilimitado' ? '∞' : `${item.total}${item.unit || ''}`}
                        </span>
                      </div>
              ...
              <div className="space-y-3">
                {integrations.map((int) => (
                  <div key={int.name} className="surface-card p-4 flex items-center justify-between">
                    ...
                    <Button variant={int.connected ? 'outline' : 'default'} size="sm" className={int.connected ? '' : 'btn-primary-gradient'}>
              ...
              <div className="surface-card p-6 space-y-4">
                {[
                  ...
                  <div key={title as string} className="flex items-center justify-between">
                    ...
                    <div className={`w-10 h-6 rounded-full transition-colors cursor-pointer flex items-center px-0.5 ${enabled ? 'bg-primary' : 'bg-border'}`}>
            ...
            <div className="space-y-6">
              ...
              <div className="surface-card p-6 space-y-3">
                {['Español', 'English', 'Português', 'Français'].map((lang, i) => (
                  <button key={lang} className={`w-full flex items-center justify-between px-4 py-3 rounded-lg transition-colors ${i === 0 ? 'bg-primary/5 border border-primary/20' : 'hover:bg-secondary border border-transparent'}`}>
